...
// ----------------------------------------------------------------------
// --- 3. Update User Details (PUT /users/:id) ---
// Requires a valid JWT token and a URL parameter matching the token's ID.
app.put("/users/:id", authenticateToken, async (request, response) => {
    const userIdFromParams = request.params.id;
    const { firstName, lastName, phoneNumber } = request.body;
    
    // 1. Security Check: Ensure the user is only modifying their own account
    if (request.user.id != userIdFromParams) {
        return response.status(403).json({ message: "Forbidden: You can only update your own account." });
    }
    ...
    queryParams.push(userIdFromParams); // Add the ID at the end
    ...
    try {
        ...
        return response.status(500).json({ message: "Server internal error. Could not update user." });
    }
});
...
app.delete("/users/:id", authenticateToken, async (request, response) => {
    ...
    // 1. Security Check: Ensure the user is only deleting their own account
    if (request.user.id != userIdFromParams) {
        return response.status(403).json({ message: "Forbidden: You can only delete your own account." });
    }
    ...
    try {
        ...
        return response.status(200).json({ 
            ...
            deletedUserId: userIdFromParams 
        });

    } catch (error) {
        console.error("User delete error:", error);
        return response.status(500).json({ message: "Server internal error. Could not delete user." });
    }
});
